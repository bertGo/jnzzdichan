...
<div class="banner">
    <div class="swiper-container">
        ...
        <!--todo-->
            <?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"get\" data=\"op=get&tag_md5=de9cf6b2fbd25d1c8b1a7b8026ca1bff&sql=SELECT+pos.width%2Cpos.height%2Cchild.setting+FROM+v9_poster_space+AS+pos+INNER+JOIN+v9_poster+AS+child+ON+child.spaceid+%3D+pos.spaceid+WHERE+pos.spaceid+%3D+11+ORDER+BY+child.id&return=data\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">编辑</a>";}pc_base::load_sys_class("get_model", "model", 0);$get_db = new get_model();$r = $get_db->sql_query("SELECT pos.width,pos.height,child.setting FROM v9_poster_space AS pos INNER JOIN v9_poster AS child ON child.spaceid = pos.spaceid WHERE pos.spaceid = 11 ORDER BY child.id LIMIT 20");while(($s = $get_db->fetch_next()) != false) {$a[] = $s;}$data = $a;unset($a);?>
            ...
            <?php $imageurl=$r->imageurl;$url=$r->linkurl?$r->linkurl:'';?>
                <div class="swiper-slide">
                    <a href="<?php echo $url;?>"><img src="<?php echo $imageurl;?>" /> </a>
                </div>
            <?php $n++;}unset($n); ?>
...
<div class="exhibition_top">
    <div class="ex_top-wrap">
            <div class="introduce">
                <?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"get\" data=\"op=get&tag_md5=f81d8da19d1a15e4631595cd92d91a83&sql=SELECT+new.url+%2Cnew.description%2C+new.title+FROM+v9_news+AS+new+INNER+JOIN+v9_news_data+AS+data+ON+data.id+%3D+new.id+WHERE+new.catid+%3D+32+ORDER+BY+new.listorder+&return=data\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">编辑</a>";}pc_base::load_sys_class("get_model", "model", 0);$get_db = new get_model();$r = $get_db->sql_query("SELECT new.url ,new.description, new.title FROM v9_news AS new INNER JOIN v9_news_data AS data ON data.id = new.id WHERE new.catid = 32 ORDER BY new.listorder  LIMIT 20");while(($s = $get_db->fetch_next()) != false) {$a[] = $s;}$data = $a;unset($a);?>
...
<div class="exhibition_bottom">
    <div class="ex_b-wrap">
        <!--todo-->
        <?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"get\" data=\"op=get&tag_md5=a5f8f030602d0d12b5accc9b2f39c0f3&sql=SELECT+pos.width%2Cpos.height%2Cchild.setting+FROM+v9_poster_space+AS+pos+INNER+JOIN+v9_poster+AS+child+ON+child.spaceid+%3D+pos.spaceid+WHERE+pos.spaceid+%3D+13+ORDER+BY+child.id&return=data\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">编辑</a>";}pc_base::load_sys_class("get_model", "model", 0);$get_db = new get_model();$r = $get_db->sql_query("SELECT pos.width,pos.height,child.setting FROM v9_poster_space AS pos INNER JOIN v9_poster AS child ON child.spaceid = pos.spaceid WHERE pos.spaceid = 13 ORDER BY child.id LIMIT 20");while(($s = $get_db->fetch_next()) != false) {$a[] = $s;}$data = $a;unset($a);?>
        <?php $n=1;if(is_array($data)) foreach($data AS $r) { ?>
        <?php $width = $r[width];$height=$r[height]?>
        <?php $r=current(json_decode($r[setting]));?>
        <?php $imageurl=$r->imageurl;$url=$r->linkurl?$r->linkurl:'';?>
        <a href="<?php echo $url;?>" target="_blank"><img src="<?php echo thumb($imageurl,440,190);?>"/></a>
        <?php $n++;}unset($n); ?>
        <?php if(defined('IN_ADMIN') && !defined('HTML')) {echo '</div>';}?>
    </div>
</div>
